...
final class CoreMLFaceRecognizer: @unchecked Sendable {
    ...
    private let logger = Logger(subsystem: "com.constrakr", category: "AdaFace")
    ...
    private init() {
        do {
            ...
            #if DEBUG
            print("[ConsTrakr] Face engine: AdaFace IR-18 Core ML READY")
            #endif
        } catch {
            self.model = nil
            self.isReady = false
            ...
            #if DEBUG
            print("[ConsTrakr] Face engine: AdaFace NOT loaded — \(error.localizedDescription)")
            #endif
        }
    }
