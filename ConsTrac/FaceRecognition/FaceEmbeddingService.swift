...
final class FaceEmbeddingService: @unchecked Sendable {
    private let logger = Logger(subsystem: "com.constrac", category: "FaceEmbedding")
    ...
    func generateEmbedding(
        ...
        if CoreMLFaceRecognizer.shared.isReady {
            do {
                ...
                #if DEBUG
                print("[ConsTrac][Embedding] engine=AdaFace len=\(values.count) norm=\(String(format: "%.4f", norm))")
                #endif
                return embedding
            } catch {
        ...
        #if DEBUG
        print("[ConsTrac][Embedding] engine=HandcraftedFallback len=\(values.count)")
        #endif
