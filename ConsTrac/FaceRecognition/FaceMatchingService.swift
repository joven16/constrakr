...
final class FaceMatchingService {
    ...
    private let logger = Logger(subsystem: "com.constrac", category: "FaceMatch")
    ...
    func match(
        ...
        for employee in enrolled {
            ...
            for enrolledEmb in employee.faceEmbeddings {
                ...
                #if DEBUG
                print("[ConsTrac][Match] vs \(employee.employeeCode)/\(enrolledEmb.pose.rawValue) sim=\(String(format: "%.4f", score))")
                #endif
            }
